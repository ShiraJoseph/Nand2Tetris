	...
	public static void main(String[] args) {
		...
		File inputFile = new File (args.length > 0 ? args[0]
				: "C:\\Users\\Shira\\Google Drive\\Work\\School\\Coursera\\Nand2Tetris\\nand2tetris"
						+ "\\projects\\08\\ProgramFlow\\BasicLoop");
		...
		try {
			...
			if(inputFile.getName().contains("FibonacciElement"))
				fileName="FibonacciElement/FibonacciElement.asm";
			if(inputFile.getName().contains("FibonacciSeries"))
				fileName = "FibonacciSeries.asm";
			if(inputFile.getName().contains("NestedCall"))
				fileName="NestedCall/NestedCall.asm";
			if(inputFile.getName().contains("SimpleFunction"))
				fileName="SimpleFunction.asm";
			if(inputFile.getName().contains("StaticsTest"))
